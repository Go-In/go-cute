...
# <----------------- post
    post_count_command = "SELECT SUM(`comment_count`) FROM `" + post_table_name + "` WHERE timestamp > " + timestamp + " AND user_id = " + target_id
...
# <----------------- comment
    comment_count_command = "SELECT COUNT(*) FROM `" + comment_table_name + "` WHERE `commented_id` = " + target_id

    c.execute(comment_count_command, [])

    comment_count = c.fetchone()[0]

# <----------------- result
    if post_count == None or comment_count == None:
        print(str(init_len - len(user_list)) + ' ' + target_id)
        
        continue

    print(str(init_len - len(user_list)) + ' ' + target_id + ' ' + str(bool(float(post_count) * 110.0 / 100.0 >= comment_count) \
                                                                     & bool(float(post_count) * 90.0 / 100.0 <= comment_count)) \
